...
if __name__ == '__main__':
    ...
    total_inputs = np.load(os.path.join(output_dir, 'inputs_{}_{}.npy'.format(start, end))) 
    total_inputs_rob = np.load(os.path.join(output_dir, 'inputs_rob_{}_{}.npy'.format(start, end))) 
    total_inputs_rec = np.load(os.path.join(output_dir, 'inputs_rec_{}_{}.npy'.format(start, end))) 
    ...
    for batch_idx in range(num_batches): 
        ...
        if args['attack_type_eval'].endswith('L2'):
            dists = torch.norm((inputs_adv - inputs).view(inputs.size(0), -1), dim=1) 
        elif args['attack_type_eval'].endswith('Linf'): 
            dists = torch.amax(torch.abs((inputs_adv - inputs).view(inputs.size(0), -1)), dim=1)
        else:
            dists = None

        corrects_adv = torch.logical_or(preds_adv == targets, dists > args['wbox_epsilon']).long()
