...
class AutoAttack():
    def __init__(self, model, norm='Linf', epsilon=.3, wbox_epsilon=.3, seed=None, verbose=True,
                 attacks_to_run=[], version='standard', is_tf_model=False,
                 device='cuda', log_path=None, **kwargs):
        self.model = model
        self.norm = norm
        assert norm in ['Linf', 'L2']
        self.epsilon = epsilon
        self.wbox_epsilon = wbox_epsilon
        self.seed = seed
        ...
        if version in ['standard', 'plus', 'rand', 'cheap']:
    ...
    def __call__(self, x_orig, y_orig, bs=512, wbox=False, x_o=None):
        # x_orig: initial attack image (x_s)
        # x_o: original (natural) image (x_o)
        # wbox: perform white-box attack evaluation
        if self.verbose:
        ...
        with torch.no_grad():
            ...
            for attack in self.attacks_to_run:
                ...
                for batch_idx in range(n_batches):
                    ...
                    # additional white-box attack checking
                    if wbox:
                        res = self.get_dist(adv_curr, x_o[batch_datapoint_idcs, :])
                        near_batch = res < self.wbox_epsilon
                    else:
                        near_batch = torch.ones(adv_curr.shape[0], dtype=torch.bool, device=adv_curr.device)
                    non_robust_lin_idcs = batch_datapoint_idcs[false_batch]
                    near_non_robust_lin_idcs = batch_datapoint_idcs[false_batch * near_batch]
                    
                    robust_flags[near_non_robust_lin_idcs] = False

                    x_adv[non_robust_lin_idcs] = adv_curr[false_batch].detach().to(x_adv.device)
                
                    if self.verbose:
                        num_non_robust_batch = torch.sum(false_batch * near_batch)    
            ...
            # final check
            if self.verbose:
                res = self.get_dist(x_adv, x_orig)
    ...
    def set_version(self, version='standard'):
        ...
        if version == 'standard':
            ...
            self.attacks_to_run = ['apgd-ce', 'apgd-t', 'fab-t']
            self.apgd.n_restarts = 1
            self.fab.n_restarts = 1
            self.apgd_targeted.n_restarts = 1
            self.fab.n_target_classes = 9
    ...
    def get_dist(self, x1, x2):
        if self.norm == 'Linf':
            res = (x1 - x2).abs().view(x2.shape[0], -1).max(1)[0]
        elif self.norm == 'L2':
            res = ((x1 - x2) ** 2).view(x2.shape[0], -1).sum(-1).sqrt()
        return res


class AutoAttackL2(AutoAttack):
    def __init__(self, model, epsilon=0.5, **kwargs):
        super(AutoAttackL2, self).__init__(model, norm='L2', epsilon=epsilon, **kwargs)


class AutoAttackLinf(AutoAttack):
    def __init__(self, model, epsilon=8/255, **kwargs):
        super(AutoAttackLinf, self).__init__(model, norm='Linf', epsilon=epsilon, **kwargs)
